...
class Transformer_LSTM(nn.Module):
    def __init__(self, n_classes: int = 8, d_model: int = 13,
                 n_heads: int = 1, n_attn_layers: int = 2,
                 dropout: float = 0.28, lstm_hidden: int = 8,
                 seq_len: int = 13):
        super().__init__()
        # Project scalar input -> d_model so attention has somewhere to work
    ...
    def forward(self, x):                       # x: (B, 1, 13)
        ...
        x = self.input_proj(x) + self.pos_emb   # (B, 13, 13)
        x = self.transformer(x)                 # (B, 13, 13)
        _, (h_n, _) = self.lstm(x)              # (1, B, 4)
